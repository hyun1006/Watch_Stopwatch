...
이 프로젝트는 Basys 3 FPGA 보드를 활용하여 디지털 시계(Watch)와 스톱워치(Stopwatch) 기능을 구현한 결과물입니다.
단순히 하드웨어 버튼으로만 제어하는 것을 넘어, UART 통신을 통해 PC에서 명령어를 보내 FPGA를 제어하는 기능을 탑재했습니다. 특히 물리적 스위치와 PC 제어 신호 간의 우선순위 충돌 문제를 해결하여 안정적인 동작을 보장하도록 설계되었습니다.
...
* 해결책 (Priority Logic):
    * UART로 모드 전환 명령(`'m'`)이 수신되면 내부 플래그(`pc_mode_lock`)를 High로 설정.
